...
func TestWithWindow(t *testing.T) {
	spec, err := ParseInterval("minutes")
	if err != nil {
		t.Fatalf("ParseInterval() error = %v", err)
	}
	custom, err := spec.WithWindow(90)
	if err != nil {
		t.Fatalf("WithWindow() error = %v", err)
	}
	if custom.Window != 90 {
		t.Fatalf("window = %d, want 90", custom.Window)
	}
	if len(custom.Buckets(time.Now())) != 90 {
		t.Fatalf("bucket count = %d, want 90", len(custom.Buckets(time.Now())))
	}
	if _, err := spec.WithWindow(MaxWindow + 1); err == nil {
		t.Fatal("expected error for oversized window")
	}
	unchanged, err := spec.WithWindow(0)
	if err != nil || unchanged.Window != 60 {
		t.Fatalf("zero window should keep default, got %+v err=%v", unchanged, err)
	}
}
