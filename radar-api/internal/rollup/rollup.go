...
// WithWindow returns a copy using window when it is within MinWindow..MaxWindow.
// Zero or negative leaves the default window unchanged.
func (i Interval) WithWindow(window int) (Interval, error) {
	if window <= 0 {
		return i, nil
	}
	if window < MinWindow || window > MaxWindow {
		return Interval{}, fmt.Errorf("window must be between %d and %d", MinWindow, MaxWindow)
	}
	i.Window = window
	return i, nil
}
