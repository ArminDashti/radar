...
.DESCRIPTION
  Remote install script for .armin/docker-scripts/install-on-docker-server.yaml.
...
USAGE:
  .\.armin\docker-scripts\install-on-docker-server.ps1
  .\.armin\docker-scripts\install-on-docker-server.ps1 -Stop
...
CONFIG:
  ...
  delete_volume       ignored — install always wipes this stack’s volumes/DB
  delete_image        ignored — install always removes this stack’s image
...
NOTES:
  - Fresh install always: compose down -v, remove image_tag, remove leftover containers/volumes for this stack.
  - Optional -Stop also wipes volumes/image for this stack.
...
try {
    ...
    if ($Stop) {
        ...
        Invoke-Remote -Target $target -RemoteCommand "docker compose -p '$stackName' $composeFilesArg --project-directory '$volumeDir' down -v >/dev/null 2>&1 || docker compose -p '$stackName' down -v >/dev/null 2>&1 || true"
    ...
    if ($usePublishOverlay) {
        Write-Step "Sync $publishComposeName for wipe/up"
        Copy-ToRemote -Target $target -LocalPath (Get-LongFilePath $publishComposePath) -RemotePath "$volumeDir/$publishComposeName"
    }
    if ($useOpenrouterProxyOverlay) {
        Write-Step "Sync $openrouterProxyComposeName for wipe/up"
        Copy-ToRemote -Target $target -LocalPath (Get-LongFilePath $openrouterProxyComposePath) -RemotePath "$volumeDir/$openrouterProxyComposeName"
    }
    ...
    if ($buildImageOn -eq 'server') {
        ...
        Copy-ToRemote -Target $target -LocalPath (Get-LongFilePath $yamlLocal) -RemotePath "$volumeDir/install-on-docker-server.yaml"
    }
