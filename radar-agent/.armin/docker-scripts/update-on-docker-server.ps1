...
.DESCRIPTION
  Remote update script for .armin/docker-scripts/update-on-docker-server.yaml.
...
USAGE:
  .\.armin\docker-scripts\update-on-docker-server.ps1
  .\.armin\docker-scripts\update-on-docker-server.ps1 -Stop
...
try {
    ...
    if ($buildImageOn -eq 'server') {
        ...
        $syncItems = @(
            ...
            @{ Local = (Join-Path $DeployDir 'update-on-docker-server.yaml'); Remote = "$volumeDir/update-on-docker-server.yaml"; Label = 'update-on-docker-server.yaml' }
        )
        if ($usePublishOverlay) {
