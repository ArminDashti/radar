...
ICMP uses the OS `ping` command (one echo: `ping -n 1` on Windows, `ping -c 1`
on Linux), so reported times match a normal terminal ping. Ensure `ping` is
installed and on `PATH` (Alpine images use the `iputils` package).
