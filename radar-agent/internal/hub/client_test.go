...
func TestSubmitPermanentErrorIsNotWrappedAway(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, `{"error":"host is inactive or protocol is disabled"}`, http.StatusBadRequest)
	}))
	defer server.Close()

	client := NewClient(server.URL, "secret", time.Second)
	err := client.Submit(context.Background(), []Sample{{EndpointID: 1, Protocol: "http", ObservedAt: time.Now()}})
	if !IsPermanent(err) {
